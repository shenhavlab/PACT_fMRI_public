function [out_G, out_C, out_T, out_R, out_S, out_absR]=pcm_estGCrossval(B, partition, conditionVec, varargin)
...
for i=1:numPart
    ...
    % Left-out partition
    indxA = ismember(partition, part(:,i));
    Za    = Z(indxA,:);
    ...
    indxB = ~ismember(partition, part(:,i));
    Zb    = Z(indxB,:);
    ...
    A(interestA,:)    = a(1:length(interestA),:);
    nA(interestA,i)   = 1;
    ...
    % ===== calc similarity  =====
    G(:,:,i) = (AA*BB');                                % 2nd moment (normalized by n)
    C(:,:,i) = (center(AA)*center(BB)');                % covariance (normalized by n)
    T(:,:,i) = (AA./vecnorm(AA))*(BB./vecnorm(BB))';    % cosine

    R(:,:,i) = corr(AA', BB');                          % correlation
    S(:,:,i) = corr(AA', BB', "type","Spearman");       % spearman correlation
    absR(:,:,i) = corr(abs(AA)', abs(BB)');             % abs correlation
...
out_G = sum(G,3)./nPart; out_G = (out_G + out_G')/2;   % mean 2nd moment
out_C = sum(C,3)./nPart; out_C = (out_C + out_C')/2;   % mean cosine
out_T = sum(C,3)./nPart; out_T = (out_T + out_T')/2;   % mean cosine

out_R = sum(R,3)./nPart; out_R = (out_R + out_R')/2;   % mean correlation
out_S = sum(S,3)./nPart; out_S = (out_S + out_S')/2;   % mean spearman correlation
out_absR = sum(absR,3)./nPart; out_absR = (out_absR + out_absR')/2;   % mean abs correlation
