...
for s=1:length(Y)
    G_hat(:,:,s),out_R(:,:,s)=pcm_estGCrossval(Y{s},partVec{s},condVec{s}); 
end; 
...
% [Tgroup,theta] = pcm_fitModelIndivid(Y,M,partVec,condVec,'runEffect',runEffect);
...
% [Tgroup,theta] = pcm_fitModelIndividCrossval(Y,M,partVec,condVec,'runEffect',runEffect);
