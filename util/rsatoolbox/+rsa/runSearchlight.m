...
Opt.idealBlock     = 1e9;    % Ideal block size, increase this if you have more free memory
% Opt.idealBlock     = 1e7;    % Ideal block size, increase this if you have more free memory

Opt = rsa.getUserOptions(varargin,Opt);

% directory
[d f t m]=spm_fileparts(outFiles{1});
delete(sprintf('%s/temp*.mat', d));
...
while (~isDone)
    ...
    save(sprintf('%s/temp_%2.2d.mat', d, b),'T');
    isCalc(j) = true;
    isDone    = all(isCalc);
    b=b+1;
end
...
Result=nan(numOutfiles,numVoxel);
...
for b=1:numBlocks
    ...
    [sectionTic, totalTic] = deal(tic);

    load(sprintf('%s/temp_%2.2d.mat', d, b));
    ...
    parResult=nan(numOutfiles,size(T.LI,1));
    for i = 1:size(T.LI,1)

        if (Opt.transposeInput)
            parResult(:, i) = feval(rsaFunction, full(X(T.LI{i},:)), Opt.optionalParams{:});
        else
            parResult(:, i) = feval(rsaFunction, full(X(T.LI{i},:)'), Opt.optionalParams{:});
        end

        if mod(i, round(size(T.LI,1)/10)) == 0
            fprintf('---- (finished vx %d/%d). section dur: %.2fm, total dur: %.2fm\n', i, size(T.LI,1), toc(sectionTic)/60, toc(totalTic)/60);
            sectionTic = tic;
        end
    ...
    % fill-in results
    Result(:, T.j) = parResult;

    % print
    fprintf('(finished block %d/%d). block dur: %.2fh\n', b, numBlocks, toc(totalTic)/3600);
    spm_progress_bar('Set',b);

end

% Clean up the searchlights
delete(sprintf('%s/temp*.mat', d));
...
for i=1:size(Result,1)
    Z=zeros(VolIn(1).dim); % zeros for fsl
